...
/**
 * Generates XML sitemap for song pages.
 ...
 * @example
 * // GET /sitemap/songs/1.xml
 * // Returns XML sitemap for page 1 of songs
 */
export default defineEventHandler(async (event) => {
  const params = getRouterParams(event)
  const page = Number.parseInt(params['page.xml'].replace('.xml', ''))

  const urls_ = await $fetch<string[]>(`/api/__sitemap__/songs?page=${page}`)
