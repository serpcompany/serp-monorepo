...
/**
 * Generates XML sitemap for artist pages.
 ...
 * @example
 * // GET /sitemap/artists/1.xml
 * // Returns XML sitemap for page 1 of artists
 */
export default defineEventHandler(async (event) => {
  const params = getRouterParams(event)
  const page = Number.parseInt(params['page.xml'].replace('.xml', ''))

  const urls_ = await $fetch<string[]>(`/api/__sitemap__/artists?page=${page}`)
