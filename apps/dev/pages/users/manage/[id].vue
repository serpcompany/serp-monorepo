...
const {
  ...
  error: entityError,
} = await useFetch<Record<string, unknown>>(`/api/entity/${entityId}`)
...
} = await useFetch<{ edits: unknown[] }>(`/api/entity/edit?id=${entityId}`)

const dynamicFields = computed(() => {
  if (!entityData.value)
    return []
  return Object.keys(entityData.value).filter(
    k =>
      ![
        'id',
        'domain',
...
watchEffect(() => {
  ...
  dynamicFields.value.forEach((key) => {
    if (!(key in newEdit)) {
      if (key === 'categories' && entityData.value[key]) {
        newEdit[key] = entityData.value[key].map((c: unknown) => c.name)
      }
      else if (key === 'topics' && entityData.value[key]) {
        newEdit[key] = entityData.value[key].map((t: unknown) => t.name)
      }
      else {
        newEdit[key] = Array.isArray(entityData.value[key]) ? [] : ''
      }
    }
  })
})

const categoriesList = await useEntityCategories(entityData.module)
const categoryOptions = categoriesList.length
...
const topicsList = await useEntityTopics(entityData.module)
const topicOptions = topicsList.length ? topicsList.map(t => t.name) : []
...
<template>
  <div class="space-y-6 p-4">
    <h1 class="text-2xl font-bold">
      Manage Entity {{ entityData?.name }}
    </h1>

    <!-- Entity Loader/Error -->
    <div v-if="entityPending" class="py-8 text-center">
      Loading entity…
    </div>
    <div v-else-if="entityError" class="py-8 text-center text-red-500">
    ...
    <section>
      <h2 class="mb-4 text-xl">
        Existing Edit Proposals
      </h2>
      <div v-if="editsPending" class="py-8 text-center">
        Loading edits…
      </div>
      ...
      <div v-else>
        <div v-if="editsData?.edits.length" class="space-y-4">
          <UCard v-for="edit in editsData.edits" :key="edit.id" class="p-4">
            ...
            <div v-if="user?.id === entityData?.verification">
          ...
          <div v-for="key in dynamicFields" :key="key">
            <UFormField :label="humanize(key)">
              ...
              <div v-if="entityData[key]">
                ...
                >
                  Current: {{ entityData[key].map((c) => c.name).join(', ') }}
                </p>
                <p
                  v-else-if="key === 'topics'"
                  class="mt-1 text-xs text-neutral-500"
                >
                  Current: {{ entityData[key].map((t) => t.name).join(', ') }}
                </p>
                <p v-else class="mt-1 text-xs text-neutral-500">
                  Current: {{ formatValue(entityData[key]) }}
                </p>
              </div>
            </UFormField>
          </div>
          <div class="text-right">
            <UButton type="primary" :loading="loadingNew">
              Submit Edit
            </UButton>
          </div>
        </form>
