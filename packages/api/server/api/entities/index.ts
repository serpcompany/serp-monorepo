...
/**
 * Handles GET requests for entities with pagination, filtering, and sorting.
 * Supports caching and user-specific data like votes and subscriptions.
 *
 * @param {H3Event} event - The event object containing request data
 * @returns {Promise<{entities: Entity[], pagination: Pagination, category: Category | null}>}
 * @throws {Error} If validation fails or database query errors occur
 * @example
 * // GET /api/entities?page=1&limit=10&categorySlug=ai-tools
 * // Returns paginated entities with user votes and subscription status
 */
export default defineEventHandler(async (event) => {
  try {
    ...
    if (value && typeof value === 'object' && 'entities' in value) {
      const cacheValue = value as { entities: Entity[], pagination: Pagination, category: any }
      const ids = cacheValue.entities.map((e: { id: number }) => e.id)
      ...
      const entities = cacheValue.entities.map((e: Entity) => ({
        ...e,
        ...aggsById[e.id],
    ...
    const cacheResponse = {
      entities: entities.map(({ usersCurrentVote: _usersCurrentVote, ...rest }) => rest),
      pagination,
      category: category_,
    }
