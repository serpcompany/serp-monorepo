...
export async function getAllTeams(limit = 50, offset = 0) {
  const teams = await getDb()
    .select()
    .from(team)
    .limit(limit)
...
export async function createTeam(payload: InsertTeam) {
  try {
    const [team_] = await getDb()
      .insert(team)
      .values(payload)
...
export async function inviteTeamMember(payload: InsertTeamInvite) {
  try {
...
export async function updateInviteStatus(inviteId: string, status: string, userId?: string) {
...
export async function acceptTeamInvite(invite: TeamInvite, userId: string) {
  try {
    await getDb()
      .insert(teamMember)
...
export async function findTeamInvite(inviteId: string) {
  const [invite] = await getDb()
    .select()
    .from(teamInvite)
    .where(eq(teamInvite.id, inviteId))
...
export async function updateTeamInvite(inviteId: string, payload: Partial<TeamInvite>) {
  await getDb()
    .update(teamInvite)
    .set(payload)
    .where(eq(teamInvite.id, inviteId))
...
export async function checkSlugConflict(userId: string, slug: string) {
  try {
